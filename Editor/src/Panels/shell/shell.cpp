...
void Shell::command(std::string command)
{
	...
	if(type == static_cast<uint8_t>(Commands::help))
	{
		if(type == static_cast<uint8_t>(Commands::clear))
			print(Ak::ELTMcontext::getText("Console.helpClear"), 2);
		else if(type == static_cast<uint8_t>(Commands::history))
			print(Ak::ELTMcontext::getText("Console.helpHistory"), 2);
		else if(type == static_cast<uint8_t>(Commands::build))
			print(Ak::ELTMcontext::getText("Console.helpBuild"), 2);
		else if(type == static_cast<uint8_t>(Commands::sysShell))
			print(Ak::ELTMcontext::getText("Console.helpSysShell"), 2);
		else if(type == static_cast<uint8_t>(Commands::quit))
			print(Ak::ELTMcontext::getText("Console.helpQuit"), 2);
		else if(type == static_cast<uint8_t>(Commands::error))
			print("help: " + Ak::ELTMcontext::getText("errors.consoleUnknownCommand"), 1);
		else
			print(" " + Ak::ELTMcontext::getText("Console.help"), 2);
	}
	else if(type == static_cast<uint8_t>(Commands::clear))
		_out.clear();
	else if(type == static_cast<uint8_t>(Commands::history)) {}
	else if(type == static_cast<uint8_t>(Commands::build)) {}
	else if(type == static_cast<uint8_t>(Commands::sysShell)) {}
		...
		_quit = true;
	else if(type == static_cast<uint8_t>(Commands::easterEgg))
