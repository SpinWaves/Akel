    ...
    if(ImGui::BeginChild("Scrolling", ImVec2(0, _height - 75), true))
    {
        ...
        if(ImGui::BeginTable("3ways", 2, flags))
        {
            ...
            for(auto it = _texts.begin(); it!= _texts.end(); it++)
            {
                ...
                ImGui::TableNextColumn();
                if(it->second.find('\n') != std::string::npos)
                    ImGui::InputTextMultiline(std::string("##" + it->first).c_str(), it->second.data(), it->second.capacity(), {0, 0}, ImGuiInputTextFlags_CtrlEnterForNewLine | ImGuiInputTextFlags_CallbackResize | ImGuiInputTextFlags_EnterReturnsTrue, InputCallback, (void*)&it->second);
                else
