...
void ELTM_editor::onUpdate(Ak::Maths::Vec2<int>& size)
{
    if(!_input_buffer->empty())
    {
        ...
        _file = *_input_buffer;
        _texts.clear();
        _texts = _loader->getCurrentTexts();
        _modules.clear();
        _modules = _loader->getCurrentModules();
    }
    if(!_is_open)
        return;
    ...
    if(*_save != 0)
    {
        ...
        if(out.is_open())
        {
            ...
            for(auto it = _texts.begin(); it!= _texts.end(); it++)
                out << "set " << it->first << " = " << it->second << std::endl;

            for(auto it = _modules.begin(); it!= _modules.end(); it++)
            {
                out << "\nbegin module " << it->first << std::endl;
                for(auto it2 = it->second.begin(); it2 != it->second.end(); it2++)
                    out << "\tset " << it2->first << " = " << it2->second << std::endl;
    ...
    if(ImGui::BeginChild("Scrolling", ImVec2(0, _height - 75), true))
    {
        ...
        if(ImGui::BeginTable("3ways", 2, flags))
        {
            ...
            for(auto it = _texts.begin(); it!= _texts.end(); it++)
            {
                ...
                ImGui::TableNextColumn();
                ImGui::InputText(std::string("##" + it->first).c_str(), it->second.data(), it->second.capacity(), ImGuiInputTextFlags_CallbackResize, InputCallback, (void*)&it->second);
            ...
            for(auto it = _modules.begin(); it!= _modules.end(); it++)
            {
                ...
                if(isopen)
                {
                    for(auto it2 = it->second.begin(); it2 != it->second.end(); it2++)
                    {
                        ...
                        ImGui::TableNextColumn();
                        ImGui::InputText(std::string("##" + it2->first).c_str(), it2->second.data(), it2->second.capacity(), ImGuiInputTextFlags_CallbackResize | ImGuiInputTextFlags_EnterReturnsTrue, InputCallback, (void*)&it2->second);
