...
namespace Ak
{
	LuaLoader::LuaLoader() : ScriptLoader()
	{
	...
	Script LuaLoader::loadScript(std::filesystem::path lua_file)
	{
		...
		{
			Core::log::report(ERROR, "Lua Loader : invalid script path, '%s'", path.string().c_str());
			return;
		}
		
		Script script;
		sol::load_result sol_script;

		sol_script = _state.load_file(path.string());
	}
	...
	void LuaLoader::bindInputs(Input& in)
	{
