...
namespace Ak
{
	struct AK_API PipelineDesc
	{
		...
		inline constexpr bool operator==(const PipelineDesc& other) noexcept
		{
			...
			for(int i = 0; i < shaders.size(); i++)
			{
				...
				if(shaders[i]._byte_code != other.shaders[i]._byte_code)
					same_shaders = false;
			}
	...
	{
		public:
			void init(class RendererComponent& renderer, const PipelineDesc& desc);
			void destroy() noexcept;

			inline std::vector<Shader>& getShaders() override { return _desc.shaders; }
			...
			inline const PipelineDesc& getDescription() noexcept { return _desc; }
